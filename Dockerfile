...
# Add build argument for Maven profile with default value
...
COPY . .
# compile the application inside the "app" folder to generate the war file using the specified profile
...
COPY --from=builder /app/target/cs122b-winter25-jasontran.war /usr/local/tomcat/webapps/cs122b-winter25-jasontran.war
